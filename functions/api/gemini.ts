...
// 模型配置
const MODELS = {
  ANALYSIS: 'gemini-3-pro-preview',      // 用于分析、设计、评审
  GENERATION: 'gemini-3-pro-image-preview', // 用于图像生成
};
...
export function buildDesignPrompt(person: any, photoType: string): string {
...
export async function onRequestPost({ request, env }: { request: Request; env: Env }) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'review': {
        // 使用 gemini-3-pro-preview 审核（对比原图和生成图）
        ...
        try {
          ...
          result = {
            ...
            overallScore: 75,
            approved: true,
            summary: 'Generated image reviewed (JSON parse fallback)',
            recommendations: []
          };
        }
        break;
      ...
      case 'reviewPrompt': {
        // 评审Prompt质量
        const { originalImage, photoType, prompt, iteration } = data;
        
        const reviewPromptText = `Review this prompt for generating a professional ${photoType} photo.

Original Image Reference: (see attached)
Target Pose: ${photoType}
  ...
  "iteration": ${iteration}
        ...
        try {
          ...
          result = {
            ...
            approved: true,
            summary: 'Prompt review completed',
            strengths: ['Clear pose specification'],
            weaknesses: [],
            suggestions: [],
      ...
      case 'reviewResult': {
        // 评审生成结果质量
        const { originalImage, photoType, generatedImage, iteration } = data;
        
        const reviewPrompt = buildComparisonPrompt(originalImage, generatedImage, {}, photoType);
        
        const parts: any[] = [{ text: reviewPrompt }];
        ...
        try {
          ...
          result = {
            ...
            facialFeatures: { preservationScore: 75, matchingFeatures: [], differences: [] },
            qualityAssessment: { professionalism: 80, beautification: 75, lighting: 85, pose: 80 },
            overallScore: 75,
            approved: true,
            summary: 'Image review completed',
            recommendations: [],
            iteration: iteration || 1
          };
        }
